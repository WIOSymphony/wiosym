---
output: github_document
---

```{r, include = FALSE}
library(gsheet)
library(dplyr)
library(tidyr)
library(knitr)
ver <- "2.1"
```

```{r, include = FALSE}
layers <- gsheet2tbl("https://docs.google.com/spreadsheets/d/1JIV92XisuBctBoVT53yqj0orPNRVyRQVEDtMq7R0fKw/edit?usp=sharing")
layers <- filter(layers, is.na(file_name) == FALSE)
eco <- filter(layers, theme == "Ecosystem")
pres <- filter(layers, theme == "Pressure")
```

# Ecosystem components
```{r, include = FALSE}
n <- eco$layer_file_name
raw <- paste0("[Download]", "(https://github.com/WIOSymphony/wiosym/raw/main/products/v", ver, "/output_geotiffs/eco/", n, ".tif)")
eco$raw_link <- raw
eco$source <- "Source 1 name [URL](www.google.com), Source 2 name [URL](www.google.com), etc"

eco <- select(eco, title, theme, raw_link, source)
colnames(eco) <- c("Layer", "Theme", "Download link", "Data source(s)")
```

```{r, echo = FALSE}
kable(eco)
```

# Pressures
```{r, include = FALSE}
n <- pres$layer_file_name
raw <- paste0("[Download]", "(https://github.com/WIOSymphony/wiosym/raw/main/products/v", ver, "/output_geotiffs/pres/", n, ".tif)")
pres$raw_link <- raw
pres$source <- "Source 1 name [URL](www.google.com), Source 2 name [URL](www.google.com), etc"

pres <- select(pres, title, theme, raw_link, source)
colnames(pres) <- c("Layer", "Theme", "Download link", "Data source(s)")
```

```{r, echo = FALSE}
kable(pres)
```
