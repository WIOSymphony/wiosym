    ...
    for i1 in range(n_steps): # time loop
        ...
        def add_OSPAR_results(year_current,month_current,key_,df_results):
            ...
            if mask_select.sum() > 0 and mask_beach_cleanup.sum() > 0:
                ...
                assert(len(size_) == 1)
#                 if key_ == 'OSPAR_1km':
#                     size_ = .4 #otherwise 0.8 is taken as the lower size limit, which is quite off from 0.5; assume visual beach cleanups included objects >.41m which seems fair
                
                mask_size_class =  l_arr > size_ # mask_beach_macro.copy() #l_arr > size_
        ...
        mask_select = (data_Eriksen['Year'] == year_current) & (data_Eriksen['Month'] == month_current)
        if mask_select.sum() > 0:

            mask_size_class = mask_Eriksen.copy() 

            if mask_size_class.sum() > 0:
                h3_selection = data_Eriksen.loc[(mask_select),'h%i'%h3_grid_res] 

                tracer_total = tracer_matrix_num[map_h3_to_mat[h3_selection]][:,mask_size_class].sum(axis=1)
                tracer_concentration = tracer_total / dict_h3_normalize[h3_selection]        
                tracer_concentration_meas = data_Eriksen['measurementValue_vol_5m'][(mask_select)]

                size_min = l_arr[mask_size_class][-1]
                size_max = l_arr[mask_size_class][0]
                ...
                df_tmp = pd.DataFrame({'Year': year_current,
                                       ...
                                       'l_min': size_min,
                                       'l_max': size_max,
                                       'ParentEventID': data_Eriksen['ParentEventID'][(mask_select)].values,
                                       'index_parent':data_Eriksen.index[(mask_select)].values,
                                       'Depth': data_Eriksen['Depth'][(mask_select)].values,
                                       'detection_limit': 0})
                df_results = pd.concat((df_results,df_tmp), ignore_index=True)   
...
if __name__ == '__main__':
    ...
    p.add_argument('-n_e', '--n_e', default=33, type=int, help='size of the ensemble')
    ...
    df_prior = forward_model(0*np.ones(len(mu_parameters)))
    ...
    min_val_fill = 1e-15
    # sigma_data = df_prior.loc[mask_usable_data,'var_log10']
    ...
    variance_param = np.ones(n_param) #sigma_parameters**2 #TODO: check this, should be variance of scaled parameters (=1)
    C_D = np.diag(variance_data)
    # X_data = np.random.random(n_data)
    # Y_data = np.random.random(n_data)
    # data_true = fn_find(param_true) + np.random.normal(loc=0,scale=sigma_data)
    
    #forecast (prior), analysis (posterior)
    param_f = np.zeros([n_param, n_ensemble]) #Prior ensemble
    data_f = np.zeros([n_data, n_ensemble]) #Prior ensemble
    
    param_a = np.zeros([n_param, n_ensemble])
    data_a = np.zeros([n_data, n_ensemble])
    
    if n_iter > 1:
        # alpha = 2**(n_iter - np.arange(n_iter)) #geometric series denominator
        # alpha = alpha * (np.sum(1/alpha)) #normalize to 1
        alpha = n_iter*np.ones(n_iter)
    else:
        alpha = [1.]
    
    #predefined coefficients Reynolds 2013:
    if n_iter == 4:
    ...
    for i in range(n_ensemble):
        param_f[:,i] = np.random.normal(np.zeros(n_param),(1/n_sigma_param)*np.ones(n_param))

    def run_parallel(param_f,data_f,dict_ESMDA,i1=0):
    ...
    #     mask_0 = (modelled_vals == 0)
    #     if mask_0.sum() > 0:
    #         print('Zeros found in modelled values, setting to minval %f' % min_val_fill)
    #         modelled_vals[mask_0] = min_val_fill
    #     data_f[:,i] = np.log10(modelled_vals)  
            
    #     dict_ESMDA['data']['log10_fore_0_%i' % i] = data_f[:,i]
    #     dict_ESMDA['parameters']['fore_0_%i' % i] = param_f[:,i]
    # pool.close()    
    gc.collect()
    data_f,dict_ESMDA = run_parallel(param_f,data_f,dict_ESMDA) 
    gc.collect()
    
    n_cols = 3
    n_rows = int(np.ceil(n_param/n_cols))
    
    time_ = print_time(time_,'for initial ensemble',div_by=60)
    
    for i1 in range(n_iter):
        
        print('--------------------------------------------------------------')
        print('----------------------Starting with ensemble %i----------------' % i1)
        print('--------------------------------------------------------------')

        param_mean = param_f.mean(axis=1)
        data_mean = data_f.mean(axis=1)    
        
        C_MD = np.zeros([n_param,n_data])
        C_DD = np.zeros([n_data,n_data])
        ...
        # Kalman Gain matrix:
        KG = np.dot(C_MD,np.linalg.inv(C_DD + alpha[i1]*C_D)) #TODO: or just sigma_data like in example ??
        mean_mismatch_f = 0
        mean_mismatch_a = 0
        for i2 in range(n_ensemble):
    
            z_d = np.random.normal(0,1,n_data)
            data_perturbed = data_measured + np.sqrt(alpha[i1])*np.sqrt(np.diag(C_D))*z_d
            
            mismatch = data_perturbed - data_f[:,i2]
            
            #set values below the detection limit to have a mismatch of 0
            mask_below_detection_lim = (data_f[:,i2] < data_perturbed) & (mask_detection_limit)
            mismatch[mask_below_detection_lim] = 0.
           
            # values with a fishing fraction above the measured values are ok (since small fragments cannot be identified)
            mask_above_fis_frac = (data_f[:,i2] > data_perturbed) & (mask_fis_frac)
            mismatch[mask_above_fis_frac] = 0.
    
            mean_mismatch_f += np.sum(mismatch**2)
            
            # forecast -> analysis
            param_a[:,i2] = param_f[:,i2] + np.dot(KG,mismatch)
        ...
        # # -----------------parallel processing-----------------
        # pool = multiprocessing.Pool(processes=n_parallel)
        # output_parallel = pool.map(forward_model, param_a.T)

        #     # analysis data
        #     df_member = output_parallel[i2] #forward_model(param_a[:,i2])
        ...
        #         print('Zeros found in modelled values, setting to minval %f' % min_val_fill)
        #         modelled_vals[mask_0] = min_val_fill
        #     data_a[:,i2] = np.log10(modelled_vals)  
            
        #     # data_a[:,i2] = np.log10(df_member.loc[mask_usable_data,'Modelled'].values)
        #     # mask_0 = (data_a[:,i] == 0)
        #     # if mask_0.sum() > 0:
        #     #     print('Zeros found in modelled values, setting to minval %f' % min_val_fill)
        #     #     data_a[mask_0,i] = min_val_fill
            
        #     dict_ESMDA['data']['log10_fore_%i_%i' % (i1+1,i2)] = data_a[:,i2]
        #     dict_ESMDA['parameters']['fore_%i_%i' % (i1+1,i2)] = param_a[:,i2]
        #     # data_a[:,i2] = fn_find(param_a[:,i2])
        
        data_a,dict_ESMDA = run_parallel(param_a,data_a,dict_ESMDA,i1=i1+1) 
        gc.collect()
        
        for i2 in range(n_ensemble):
            mismatch_a = data_measured - data_a[:,i2]
            mask_below_detection_lim = (data_a[:,i2] < data_measured) & (mask_detection_limit)
            mismatch_a[mask_below_detection_lim] = 0.
            
            mask_above_fis_frac = (data_a[:,i2] > data_perturbed) & (mask_fis_frac)
            mismatch[mask_above_fis_frac] = 0.
            
            mean_mismatch_a += np.sum(mismatch_a**2)
        # pool.close()
        
        mean_mismatch_f /= n_ensemble
        mean_mismatch_a /= n_ensemble
        print('Mismatch: %3.3f -> %3.3f' % (mean_mismatch_f,mean_mismatch_a))
        time_ = print_time(time_,'for ensemble %i'%i1,div_by=60)
        # new forecast is old analysis
        param_f = param_a.copy() 
        data_f = data_a.copy()
    
        param_a_mean = param_a.mean(axis=1)
        param_a_std = param_a.std(axis=1)
        
        if i1 == n_iter-1:
            df_final = forward_model(param_a_mean,final_run=True)
            dict_ESMDA['data']['log10_final'] = np.log10(df_final.loc[mask_usable_data,'Modelled'].values)
            dict_ESMDA['parameters_final_mean'] = param_a_mean
            dict_ESMDA['parameters_final_std'] = param_a_std
            
        fig1,ax1 = plt.subplots(n_rows,n_cols)
        for i3 in range(n_param):
            ...
            gauss_prior = Gaussian(x_plot,0.,1.)
            gauss_post = Gaussian(x_plot,param_a_mean[i3],param_a_std[i3])
            ax1[row_,col_].fill_between(x_plot,np.zeros(len(x_plot)),gauss_prior,color='b',alpha=.2)
            ax1[row_,col_].fill_between(x_plot,np.zeros(len(x_plot)),gauss_post,color='r',alpha=.2)
    
            # ax1[row_,col_].plot( [param_a_mean[i3],param_a_mean[i3]], [0,1], 'r--',label='upd')
            ax1[row_,col_].plot( [param_a_mean[i3],param_a_mean[i3]], [0,1], 'rd-',label='update')
            # ax1[row_,col_].plot( [param_true[i3],param_true[i3]], [0,1], 'k-', label='truth')
            ax1[row_,col_].plot( [0.,0.], [0,1], 'b--', label='prior')
    
            ax1[row_,col_].set_xlim(-6,6)
            ax1[row_,col_].set_ylim(0,1)
            fig1.suptitle('Iteration %i' %(i1+1))
            
            if i3 == 0:
                fig1.legend()
        fig1.savefig(os.path.join(folder_figure,'iter_%i.pdf'%i1))
    ...
    df_final.to_csv(os.path.join(data_folder,'00_assimilation_files/analysis_%i_iter_%i_members_'%(n_iter,n_ensemble) + str_info  + str(datetime.now())[0:10] + '_' + str(datetime.now())[11:16] + '.csv'))
