...
```{r}
#grid_1km_proj <- projectRaster(grid_1km, crs = CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"))
```
...
# spec <- filter(spec, scientificName == "Caretta caretta")
# map_leaflet(spec)
```
...
```{r}
base <- grid_1km - 1
eco1 <- raster("./data/reg/eco/rept_turtle/turtle_sdm/v01/rept_turtle_turtle_sdm_Caretta_caretta_v01.0.tif")
eco2 <- raster("./data/reg/eco/rept_turtle/turtle_sdm/v01/rept_turtle_turtle_sdm_Dermochelys_coriacea_v01.0.tif")
eco3 <- raster("./data/reg/eco/rept_turtle/turtle_sdm/v01/rept_turtle_turtle_sdm_Chelonia_mydas_v01.0.tif")
...
# Here we add data from turtle nesting sites and clutch size to combine with the above data,
# which likely represents adult turtles
# This data comes from the supplementary materials de Geer et al. (2022), 
# Marine turtles of the African east coast: current knowledge and priorities for conservation and research
# and Mortimer et al. (2020), Estimates of marine turtle nesting populations in the south-west Indian Ocean indicate the importance of the Chagos Archipelago
# The data were digitized by adding coordinates to the named sites, and clutch sizes of green sea turtles were used
# A kernel density map was then produced in ArcGIS and multiplied by an inverse cost distance raster from the shoreline
```{r}
clutch <- raster("./data/reg/eco/rept_turtle/turtle_sdm/v01/proc/green_turtle_clutch_10km.tif")
mapview(clutch, col.regions = brewer.pal(11, "Spectral"))
```

# Here we assign higher values to the areas for nesting
```{r}
prod1[] <- scales::rescale(product1[], to = c(0,0.3))
clutch <- merge(clutch, base)
clutch[clutch == 0] <- NA
clutch[clutch < quantile(clutch, 0.50)] <- NA
clutch[] <- scales::rescale(clutch[], to = c(0.3,1))
clutch <- merge(clutch, base)
prod2 <- prod1 + clutch
prod2[] <- scales::rescale(prod2[], to = c(0,1))

mapview(-prod2, col.regions = brewer.pal(11, "Spectral"))
#mapview(-clutch, col.regions = brewer.pal(11, "Spectral"))
```
...
```{r}
writeRaster(prod2, dest1_path_file1, COMPRESS=LZW, overwrite=TRUE)
```
