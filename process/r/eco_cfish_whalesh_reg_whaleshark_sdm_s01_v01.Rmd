...
```{r}
# Models
write.sdm(d, filename = paste0(work1_path,"/", name1,name2, "_sdmdata"), overwrite = T)
write.sdm(m1, filename = paste0(work1_path,"/", name1,name2, "_sdmmodel"), overwrite = T)

# Variable importance
sink(paste0(work1_path,"/", name1,name2, "_VarImp.txt"))
out <- capture.output(getVarImp(m1, id = 1))
cat(out, sep="\n")
sink()

# Evaluation
sink(paste0(work1_path,"/", name1,name2, "_Evaluation.txt"))
