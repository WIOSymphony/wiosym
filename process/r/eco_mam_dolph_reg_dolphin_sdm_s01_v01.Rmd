...
# Combine sdms for threatened and all species
# Here we give the sdm for all species a value between 0 and 0.5, and for threatened species between 0.5 and 1
# We then add the maps together, where the maximum value possible is 1.5 (maximum probability of all species and threatened species)
```{r}
base <- grid_1km - 1
eco.threat <- raster("./data/glo/eco/mam_dolph/dolphin_sdm/v01/mam_dolph_dolphin_sdm_threatened_v01.0.tif")
eco.all <- raster("./data/glo/eco/mam_dolph/dolphin_sdm/v01/mam_dolph_dolphin_sdm_all_v01.0.tif")

eco.threat[eco.threat < 0.5] <- 0
eco.all[eco.all < 0.5] <- 0

eco.all[eco.all >= 0.5] <- eco.all[eco.all >= 0.5] - 0.5
eco <- eco.all + eco.threat
product1 <- eco
