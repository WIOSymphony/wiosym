...
location = "reg"   #check
theme1 <- "eco"   #check
subtheme1 <- "bfish_demersal"   #check
name1 <- "demfish_sdm"   #check
name2 <- "" #check
...
# Pleuronectiformes (Flatfishes) = 10331
```{r}
# IF ONLY ONE TAXON GROUP IS DESIRED
spec <- occurrence(taxonid = 10331,
...
```{r}
# Individual models
p1 <- predict(m1,newdata=preds,filename=paste0(work1_path,"/p1.tif"), overwrite = T, nc = 6) 
plot(p1)
```

```{r}
# Ensemble model
e1 <- ensemble(m1,newdata=preds,filename=paste0(work1_path,"/e1.tif"),setting=list(method='weighted',stat='TSS'), overwrite=T, nc=6)
...
### Evaluations and export of models
```{r}
# Models
write.sdm(d, filename = paste0(work1_path,"/", name1,"_",name2, "_sdmdata"), overwrite = T)
write.sdm(m1, filename = paste0(work1_path,"/", name1,"_",name2, "_sdmmodel"), overwrite = T)

# Variable importance
sink(paste0(work1_path,"/", name1,"_",name2, "_VarImp.txt"))
out <- capture.output(getVarImp(m1, id = 1))
cat(out, sep="\n")
sink()

# Evaluation
sink(paste0(work1_path,"/", name1,"_",name2, "_Evaluation.txt"))
...
```{r}
#base <- grid_1km - 1
eco1 <- raster("./data/reg/eco/bfish_demersal/demfish_sdm/v01/bfish_demersal_demfish_sdm_Austroglossus_pectoralis_v01.0.tif")
eco2 <- raster("./data/reg/eco/bfish_demersal/demfish_sdm/v01/bfish_demersal_demfish_sdm_Galeichthys_feliceps_v01.0.tif")
eco3 <- raster("./data/reg/eco/bfish_demersal/demfish_sdm/v01/bfish_demersal_demfish_sdm_Leiognathus_equulus_v01.0.tif")
eco4 <- raster("./data/reg/eco/bfish_demersal/demfish_sdm/v01/bfish_demersal_demfish_sdm_Lophius_vomerinus_v01.0.tif")
eco5 <- raster("./data/reg/eco/bfish_demersal/demfish_sdm/v01/bfish_demersal_demfish_sdm_Merluccius_capensis_v01.0.tif")
eco6 <- raster("./data/reg/eco/bfish_demersal/demfish_sdm/v01/bfish_demersal_demfish_sdm_Merluccius_paradoxus_v01.0.tif")
eco7 <- raster("./data/reg/eco/bfish_demersal/demfish_sdm/v01/bfish_demersal_demfish_sdm_Pleuronectiformes_v01.0.tif")
