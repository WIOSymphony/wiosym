...
#### ABOUT ####
# ========================================================================================================= #
# Purpose: Script to create WIOSym plastic data, this one is building on a arcgis process (see proc_log/*.mxd)
#
# Brief description: standardised depth data to 250m and 1km grid 

# Suggestions on future improvements: 
#
# Created: GK, sgu, 210601
...
theme1 <- theme <- "pres" # input your main theme e.g. "eco" (choose from list in "theme_folder")
...
subtheme1 <- subtheme <- "pol_plastic" # input your subtheme based on the most appropriate option from the list. If your subtheme is missing, please add new subtheme to folders.txt first (and make sure to sync with shared location for WIOSym if applicable)
...
source1_dir <- source_dir <-  "./data_raw/glo/pres/pol_plastic/lit/dm2106011106/"
source1_id <- source_id <- "dm2106011106" # cut and past your source id here
...
source_file <- "Meijer2021_midpoint_emissions/Meijer2021_midpoint_emissions.shp"  # your data file(s)
source1_path1 <- source_path <- paste(source_dir, source_file, sep="")
source_path
plastic <- indata1 <- st_read(source_path)
...
# data5: arcgis prepepared results
data5_dir <- data_dir <-  "./data/reg/pres/pol_plastic/v01/proc/"  # input your data directory
dir(data_dir) # check what data is in this directory
data_file <- "Plastic_density_mTon_year_1km.tif" # your data file
...
data_path
plastic_r <- raster(data_path) # read in your data object if appropriate and perhaps change name to more informative...
plastic_r
plot(plastic_r)
...
# PROCESS ----------------------------------------------------------------------------------------------------------------
# process is done in arcgis in this version, just naming and normalisation here for now..

# normalise data ----------------------------------------------------------------------------------------

plastic_r_mask <- mask(plastic_r, grid_1km)


plastic_r_norm01 <- plastic_r/maxValue(plastic_r)
plastic_r_norm01_mask <- mask(plastic_r_norm01, grid_1km)

plot(plastic_r_norm01_mask)

plastic_r_norm01_log <- log(1+(plastic_r_norm01_mask*100))

plastic_r_norm01_log <- plastic_r_norm01_log/maxValue(plastic_r_norm01_log)
...
scale <- "1km"
descriptive_name <- "density_mTon_year"
...
product <- plastic_r_mask # input and check your object to be written to file
...
product1_path_norm
write_tsv(file_sourcesym, paste(product_path, "_sourcesym.txt", sep=""))  # write unique sourcesym files for all products
product <- plastic_r_norm01_mask # input and check your object to be written to file
writeRaster(product, product_path, COMPRESS=LZW, datatype = 'FLT4S', overwrite=TRUE)


# for normalised products, name like this
product1_path_norm <- product_path <- paste(dest1_path, theme1, "_", subtheme1, "_", descriptive_name, "_norm01_log_", scale, "_", version, d_version, ".tif", sep="" )
product1_path_norm
write_tsv(file_sourcesym, paste(product_path, "_sourcesym.txt", sep=""))  # write unique sourcesym files for all products
product <- plastic_r_norm01_log # input and check your object to be written to file
writeRaster(product, product_path, COMPRESS=LZW, datatype = 'FLT4S', overwrite=TRUE)


plastic_r_norm01_log
