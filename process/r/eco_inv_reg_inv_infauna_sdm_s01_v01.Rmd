...
theme1 <- "eco"   #check
subtheme1 <- "inv"   #check
name1 <- "inv_infauna_sdm"   #check
name2 <- "bivalvia" #check
...
d
m1 <- sdm(Occurrence~.,data=d,methods=c('brt','svm','glm','gam')) #'brt','glm','svm','gam'
m1
...
```{r}
# Remove predictions for specific depths
# depth <- preds$depth1km
# depth <- projectRaster(depth, grid_1km)
# depth <- resample(depth, grid_1km)
# 
# 
# product1[depth > -200] <- 0
# mapview(product1, col.regions = brewer.pal(11, "Spectral"))
```
...
```{r}
#base <- grid_1km - 1
eco1 <- raster("./data/reg/eco/inv/inv_infauna_sdm/v01/inv_inv_infauna_sdm_bivalvia_v01.0.tif")
eco2 <- raster("./data/reg/eco/inv/inv_infauna_sdm/v01/inv_inv_infauna_sdm_polychaeta_v01.0.tif")
