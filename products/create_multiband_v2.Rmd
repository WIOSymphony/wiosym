...
meta <- rio::import(paste0("layers_metadata_", ver, ".xlsx"))
...
```{r}
layers <- rio::import(paste0("layers_metadata_", ver, ".xlsx")) |> dplyr::filter(theme == "Ecosystem")
...
```{r}
layers <- rio::import(paste0("layers_metadata_", ver, ".xlsx")) |> dplyr::filter(theme == "Pressure")
layer_files <- layers$file_name
layer_files[is.na(layer_files)] <- "placeholder.tif"
#layer_files <- layer_files[c(24,25)] # run for only select layers
l <- list.files(paste0(ver, "/source_layers"), recursive = T, full.names = T)
...
```{r}
grid <- raster::raster("grid/grid_1km_v01.1.tif")
place <- raster::raster("grid/grid_1km_v01.1.tif") * 0
layers <- rio::import(paste0("layers_metadata_", ver, ".xlsx")) |> dplyr::filter(theme == "Ecosystem")
...
for(i in 1:length(layers$title)){
  ...
  temp <- raster::raster(paste0(ver, "/source_layers_fixed/", layers$file_name[i]))
  temp[temp == 0] <- NA #???
  ...
  raster::plot(temp, maxpixels = ncell(temp), main = "", axes = FALSE, ylim=raster::extent(temp)[3:4], 
...
```{r, message = FALSE}
grid <- raster::raster("grid/grid_1km_v01.1.tif")
grid_proj <- raster::raster("grid/grid_1km_v01.1_proj.tif")
place <- grid_proj * 0
layers <- rio::import(paste0("layers_metadata_", ver, ".xlsx")) |> dplyr::filter(theme == "Ecosystem")
layers$file_name[is.na(layers$file_name)] <- "placeholder.tif"
layer_files <- layers$file_name
...
# Write metadata for GitHub
meta <- rio::import(paste0("layers_metadata_", ver, ".xlsx"))
fp <- paste0(ver, "/output_multiband_compress/layers_metadata_", ver, ".xlsx")
rio::export(meta, fp)
```
