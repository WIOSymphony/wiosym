...
ph_proj <- resint(ph_change, grid_1km, nit = 100, win_size = 2000, method = "cubic")
plot(ph_proj)
```
...
```{r}
ph <- ph_proj
q <- quantile(terra::values(ph, na.rm = T), probs = 0.80) # Add quantile cutoff? Default to no cutoff
ph[ph < q] <- 0
ph_quant <- ph
plot(ph_quant)
```
