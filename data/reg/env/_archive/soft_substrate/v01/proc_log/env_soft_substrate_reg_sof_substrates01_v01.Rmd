...
```{r}
subtheme <- "soft_substrate" # copy from "subtheme_folder"
```
...
## Save R Script
```{r}
name1 <- "sof_substrate" # "REPLACE_" if script need additional name... don't use unless needed to avoid duplication.
...
data1_file4 <- "bounding_box_1km.tif"
...
data3_file <- "wiosym_data_bounding_box_v01.shp" # your data file
...
plot(grid_poly)
```
## DATA 4: mask shapefile for 30degrees cut-off PROBABLY DELETE?????
```{r}
data4_dir <- data_dir <-  "./data/reg/grid/mask/v00/"  # input your data directory
data4_file <- "30_degree_lat.shp" # your data file
...
export_directory <- "./data/reg/env/soft_substrate/v01/"

#check the directories
list.dirs(source_dir)

sft_btm_files <- list.files(paste(source_dir,"hardSoftValues_byDepthZones", sep = ""), pattern = "*SoftBotm*")




for (tif_obj in sft_btm_files) {
  ...
  writeRaster(x2,filename=file.path(paste(export_directory,"Tropical_SoftBottom_Substrate_",exportfile_name,"_1km",".tif", sep = "")), overwrite=TRUE )
  writeRaster(x2_60,filename=file.path(paste(export_directory,"Temperate_SoftBottom_Substrate_",exportfile_name,"_1km",".tif", sep = "")), overwrite=TRUE )
...
write_tsv(all_sources, paste(export_directory, "Tropical_SoftBottom_Substrate_sourcesym.txt", sep=""))
write_tsv(all_sources, paste(export_directory, "Temperate_SoftBottom_Substrate_sourcesym.txt", sep=""))
```
