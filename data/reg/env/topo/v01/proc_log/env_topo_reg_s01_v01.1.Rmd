...
source1_dir <- source_dir <-  "./data_raw/reg/env/topo/gebco/gk2110122231/"
source1_id <- source_id <- "gk2110122231" # cut and past your source id here
...
source_file <- "GEBCO_12_Oct_2021_6088be3dfc79/gebco_2021_n18.0_s-42.0_w6.0_e80.0.tif"  # your data file(s)
...
dir(source_dir) # check what data is in data_raw directory
source_file <- "GEBCO_12_Oct_2021_6088be3dfc79/gebco_2021_tid_n18.0_s-42.0_w6.0_e80.0.tif"  # your data file(s)
...
writeRaster(grid_1km_na, paste(path_work, "grid_1km_na_mean_depth_gebco2021.tif", sep= ""),  overwrite=T, COMPRESS=LZW)
writeRaster(grid_250m_na, paste(path_work, "grid_250m_na_mean_depth_gebco2021.tif", sep= ""),  overwrite=T, COMPRESS=LZW)

# write to file
dstfile <- paste(path_work, "grid_250m_na_mean_depth_gebco2021.tif", sep="")
...
writeRaster(bat_250m , paste(path_work, "grid_250m_na_mean_depth_gebco2021_lzw.tif", sep=""), overwrite=F, COMPRESS=LZW)
```

```{r}
dstfile <- paste(path_work, "grid_1km_na_mean_depth_gebco2021.tif", sep="")
...
depth_250m <- raster(paste(path_work, "env_depth_mean_grid_250m_", version, ".tif", sep=""))
depth_1km <- raster(paste(path_work, "env_depth_mean_grid_1km_", version, ".tif", sep=""))
...
resolution = 250
stat <- df %>% 
  group_by( env_depth_mean_grid_250m_v01) %>% 
  summarize(n = n()) %>%
...
stat <- stat %>%
  rename(Value = env_depth_mean_grid_250m_v01)
...
product1_path <- product_path <- paste("depth_zones_250m", "_", version, d_version, ".tif", sep="" )
product_path
```
...
# note by GL 211021 - the export of this old template script is not so nice... I fixed it manually after by moving files from proc to root and fixing names...
