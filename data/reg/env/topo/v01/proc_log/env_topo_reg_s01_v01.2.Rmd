...
source1_id <- source_id <- "gk2209052026" # cut and past your source id here
...
source_file <- "GEBCO_05_Sep_2022_b56d0c269bfd/gebco_2022_n18.0_s-42.0_w6.0_e80.0.tif"  # your data file(s)
...
writeRaster(grid_1km_na, paste(path_work, "grid_1km_na_mean_depth_gebco2022.tif", sep= ""),  overwrite=T, COMPRESS=LZW)
writeRaster(grid_250m_na, paste(path_work, "grid_250m_na_mean_depth_gebco2022.tif", sep= ""),  overwrite=T, COMPRESS=LZW)

# write to file
dstfile <- paste(path_work, "grid_250m_na_mean_depth_gebco2022.tif", sep="")
...
writeRaster(bat_250m , paste(path_work, "grid_250m_na_mean_depth_gebco2022_lzw.tif", sep=""), overwrite=F, COMPRESS=LZW)
```

```{r}
dstfile <- paste(path_work, "grid_1km_na_mean_depth_gebco2022.tif", sep="")
...
resolution = 1000
stat <- df %>% 
  group_by( env_depth_mean_grid_1km_v01) %>% 
  summarize(n = n()) %>%
  mutate(km2 = round(n*resolution*resolution/1000000, 2)) %>% 
  mutate(percent = round(100 * n/sum(n), 2)) %>% 
  print()
...
stat <- stat %>%
  rename(Value = env_depth_mean_grid_1km_v01)
...
# PRODUCT 1: Bathymetry, code chunk below not used - manually experted from the proc folder to save some time...
...
export_object1 <- bathymetry_1km # REPLACE... 
export_object2 <- bathymetry_250m # REPLACE... 
