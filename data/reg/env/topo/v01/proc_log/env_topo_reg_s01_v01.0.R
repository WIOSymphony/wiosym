...
# SECTION 2 , NOT YET RUN IN V01.0 Divide depth into individual rasters with depth zones ------------------------------------------------------------------------------------
...
# write to product file: Bathymetry 1km  -------------------------------------------------------------------------------------- 

product1_path <- product_path <- paste(dest1_path, theme1, "_", subtheme1, "_depth_mean_1km", "_", version, d_version, ".tif",  sep="")
product_path
...
product <- bat_1km # input and check your object to be written to file
product
plot(product)
writeRaster(product, product_path, COMPRESS=LZW, datatype = 'INT4S', overwrite=TRUE)

# write to product file: Bathymetry 250m  -------------------------------------------------------------------------------------- 

product2_path <- product_path <- paste(dest1_path, theme1, "_", subtheme1, "_depth_mean_250m", "_", version, d_version, ".tif", sep="" )
product_path
...
# write to file 
write_tsv(file_sourcesym, paste(product_path, "_sourcesym.txt", sep=""))  # write unique sourcesym files for all products
product <- bat_250m_mask # input and check your object to be written to file
product
plot(product)
writeRaster(product, product_path, COMPRESS=LZW, datatype = 'INT4S', overwrite=TRUE)
