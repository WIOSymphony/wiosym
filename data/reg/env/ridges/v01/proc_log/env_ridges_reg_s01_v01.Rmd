...
# ABOUT
# ========================================================================================================= #
## Brief description: Mid-ocean ridges raster map was downloaded from the "blue habitats" webpage
## downloaded raster datasets that was combined and cropped to the project area
...
```{r}
subtheme <- "ridges" # copy from "subtheme_folder"
```
...
## DIR1: 
```{r}
source1_dir <- source_dir <-  "./data_raw/glo/env/geo_geom/bluehab/gk2110121513/"  # REPLACE - internal path
source1 <- source_id <- "gk2110121513" # REPLACE
...
```{r}
# Read Files1
source1_path <- path <- paste(source_dir, "global seafloor geomorphic features map/Ridges.shp", sep="")
ridges <- st_read(path) # REPLACE object_name
# add files from dir as needed
```

```{r}
# Read Files2 -->
source2_path <- path <- paste(source_dir, "global seafloor geomorphic features map/Spreading_ridges.shp", sep="") 
spreading_ridges <- st_read(path) # REPLACE object_name 
...
# Map ridges polygons to 250m raster -----------------------------------------------
```{r}


sf::sf_use_s2(FALSE) # to remove error in the intersection function caused by S2 update in "sf" package 


indata_sf <- st_intersection(ridges, grid_poly) #crop ridges data to WIOSym area
indata_sf1 <- st_intersection(spreading_ridges, grid_poly) #crop spreading ridges data to WIOSym area
indata_sf_all = st_union(indata_sf, indata_sf1,by_feature = TRUE, is_coverage = FALSE) #combine both
indata_sf_all <- indata_sf_all[,-(1:11)]#remove unnecessary colums
indata_sf_all <- dplyr::mutate(indata_sf_all, data = 1)

st_write(indata_sf_all, paste0(source1_dir, "global seafloor geomorphic features map/ridges_crop.shp"), delete_layer = TRUE)
...
path_work <- work_path
source1_path <- path <- paste(source_dir, "global seafloor geomorphic features map/ridges_crop.shp", sep="")



# write empty grid for gdalutil work
writeRaster(grid_250m_na, paste(path_work, "grid_250m_ridges.tif", sep= ""),  overwrite=T, COMPRESS=LZW)
writeRaster(grid_1km_na, paste(path_work, "grid_1km_ridges.tif", sep= ""),  overwrite=T, COMPRESS=LZW)
```
...
sf_warp <- gdalUtils::gdal_rasterize(src_datasource = source1_path,
                                     dst_filename = paste(path_work, "grid_250m_ridges.tif", sep= ""),
                                     b = 1,
                                     at = F,
                                     a = "data",
                                     output_Raster = TRUE,
...
```{r}
ridges_250m <- sf_warp
crs(ridges_250m) <- crs(grid_1km)
(ridges_250m)

plot(ridges_250m) # ready to be exported if OK
```
...
sf_warp <- gdalUtils::gdal_rasterize(src_datasource = source1_path,
                                     dst_filename = paste(path_work, "grid_1km_ridges.tif", sep= ""),
                                     b = 1,
                                     at = F,
                                     a = "data",
                                     output_Raster = TRUE,
...
```{r}
ridges_1km <- sf_warp
crs(ridges_1km) <- crs(grid_1km)
(ridges_1km)


plot(ridges_1km) # ready to be exported if OK
```
...
## read objects
```{r}
product_orig <- ridges_250m # input and check your object to be written to file
...
```{r}
descriptive_name <- "mid-ocean ridge"  # name of product, for pressure/ecosystem components select from name list above
```
## paths
```{r}
...
# read objects
```{r}
product_orig <- ridges_1km # input and check your object to be written to file
#product_norm01 <- REPLACE # input and check your object to be written to file
#product_uncertainty <- REPLACE # input and check your object to be written to file
