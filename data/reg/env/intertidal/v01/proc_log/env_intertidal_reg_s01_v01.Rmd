...
# ABOUT
# ========================================================================================================= #
## Brief description: Intertidal raster map downloaded from Intertidal_____
...
```{r}
subtheme <- "intertidal" # copy from "subtheme_folder"
```
...
## DIR1: 
```{r}
source1_dir <- source_dir <-  "./data_raw/glo/env/other/globalintertidalchange/ff2111221607/"  # REPLACE - internal path
source1 <- source_id <- "ff2111221607" # REPLACE
...
```{r}
dir.create(paste(work_path, "1km/", sep = ""))
dir.create(paste(work_path, "250m/", sep = ""))
tif_files <- c("60_0.tif", "60_20.tif" , "60_20.tif", "40_20.tif", "20_0.tif", "40_0.tif", "40_-20.tif", "0_0.tif", "20_-20.tif")

#output_rast <- list()
j = 0

for (tif_obj in tif_files) {
  a = paste(source_dir,tif_obj, sep = "")
   
   
   
   x <- gdalUtils::gdalwarp(srcfile = a, 
                            dstfile = f <- tempfile(fileext='.tif'),
                            r='mode',
                            #s_srs = "+proj=longlat +datum=WGS84 +no_defs",
                            t_srs = "+proj=longlat +datum=WGS84 +no_defs",
                            #t_srs = "+proj=cea +lat_ts=-12 +lon_0=12 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs", 
                            tr = c(0.0005, 0.0005), 
                            tap = TRUE,
                            mask = paste(data_dir, data3_file, sep = ""),
                            srcnodata = "NA",
                            output_Raster=TRUE, 
                            overwrite=TRUE,
                           )
  x <- crop(x, grid_poly) # crop rasters to project study area
  x_250m <- projectRaster(x, crs = crs(grid_250m), res = res(grid_250m))
  x_1km <- projectRaster(x, crs = crs(grid_1km), res = res(grid_1km))
  #output_rast[[tif_obj]] <- x
  writeRaster(x_250m,filename=file.path(paste(work_path, "250m/", j, "intertidal_250m.tif", sep = "")), datatype='INT1U', overwrite=TRUE )
  writeRaster(x_1km,filename=file.path(paste(work_path, "1km/", j, "intertidal_1km.tif", sep = "")), datatype='INT1U', overwrite=TRUE )
  j = j + 1
}
...
```{r}
# creating the 250m intertidal raster and 1 km intertidal raster 

mosaic_rasters(gdalfile=list.files(paste(work_path,"250m/", sep = ""), pattern = "*.tif", full.names = TRUE ),dst_dataset=file.path(work_path,"intertidal_250m.tif"))
intertidal_250m <- raster(paste(work_path,"intertidal_250m.tif", sep = ""))
mosaic_rasters(gdalfile=list.files(paste(work_path,"1km/", sep = ""), pattern = "*.tif", full.names = TRUE ),dst_dataset=file.path(work_path,"intertidal_1km.tif"))
intertidal_1km <- raster(paste(work_path,"intertidal_1km.tif", sep = ""))
...
## read objects
```{r}
product_orig <- intertidal_250m # input and check your object to be written to file
...
```{r}
descriptive_name <- "intertidal"  # name of product, for pressure/ecosystem components select from name list above
```
## paths
```{r}
...
# read objects
```{r}
product_orig <- intertidal_1km # input and check your object to be written to file
#product_norm01 <- REPLACE # input and check your object to be written to file
#product_uncertainty <- REPLACE # input and check your object to be written to file
