...
# ========================================================================================================= #
## Brief description: 
model of coral habitats using literature data (EF Nansen 2018 survey).
...
obs_env_path <- paste(work_path, "gv_for_modelling_coral_incl_env_var", ".rds", sep="" )
...
TrainClasses <- df  %>% 
  ...
  gbmGrid <- expand.grid(.n.trees = c(1300),
...
coral_crop <- crop(coral_mask, ext1)
...
coral_fill_path <- path <- paste(work_path, "coral_model_addedpoints_fill.tif", sep="")
writeRaster(coral_fill, path, overwrite=T, COMPRESS=LZW)
...
```{r}
inraster <- coral_fill
inraster_path <- coral_fill_path
inraster_path

# write empty grid for gdalutil work

gdalutil_path <- paste(work_path, "grid_1km_coral_gdalutil.tif", sep= "")
...
r <- coral_1km <- raster(dstfile)
raster(dstfile)

writeRaster(coral_1km , paste(work_path, "coral_1km_sayamodel_addedpoints_percent_crop_lzw.tif", sep=""), overwrite=F, COMPRESS=LZW)

r <- raster("./data/reg/eco/bh/v02/proc/coral_1km_sayamodel_addedpoints_percent_crop_lzw.tif")
...
writeRaster(coral_1km , paste(work_path, "coral_1km_sayamodel_percent_addedpoints.tif", sep=""), overwrite=F, COMPRESS=LZW)

coral_1km <- raster(paste(work_path, "coral_1km_sayamodel_percent_addedpoints.tif", sep=""))
...
all_sources <- data_raw_sources %>% #add_row(data_sources) %>%data_raw_sources
print(all_sources)
```
...
product_orig1 <- "coral_percent_cover_Saya"  # example

(product_path1 <- paste(dest_path, product_orig1, name1, "_", version, d_version, ".tif", sep="" ))  # Example, change the "uncertainty part as needed (norm01 for example)
