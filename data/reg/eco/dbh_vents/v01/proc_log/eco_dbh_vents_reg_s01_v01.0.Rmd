...
# ABOUT
# ========================================================================================================= #
## Brief description: Hydro thermal vents raster map downloaded (as vector points) from Pangaea website
...
```{r}
subtheme <- "dbh_vents" # copy from "subtheme_folder"
```
...
## DIR1: 
```{r}
source1_dir <- source_dir <-  "./data_raw/glo/eco/hab/pangaea/ff2111170956/"  # REPLACE - internal path
source1 <- source_id <- "ff2111170956" # REPLACE
...
```{r}
# Read Files1
source1_path <- path <- paste(source_dir, "Global_2020_HydrothermalVents_InterRidgeVentsDatabasev3.4/Global_2020_HydrothermalVents_InterRidgeVentsDatabasev3.4.shp", sep="")
hydro_vents <- st_read(path) # REPLACE object_name
# add files from dir as needed
```

<!-- ## DIR2:  -->
<!-- ```{r} -->
<!-- source2_dir <- source_dir <-  "./data_raw/.../uniqueid/"  # REPLACE - internal path -->
<!-- source2 <- source_id <- "uniqueid" # REPLACE -->
<!-- (read_tsv(paste(source_dir, source_id, "_metasym.txt", sep=""))) # check the metasym file -->
<!-- ```  -->
...
<!-- ```{r} -->
<!-- # Read Files2 -->
<!-- source2_path <- path <- paste(source_dir, "", sep="") -->
...
# Map hydro_vents polygons to 250m raster -----------------------------------------------
```{r}
indata_sf <- st_intersection(hydro_vents, grid_poly) #crop hydro_vents data to WIOSym area
plot(indata_sf)


indata_sf <- st_buffer(indata_sf, dist = indata_sf$Full.Sprea) # convert points to polygon based on area, Full_Sprea
plot(indata_sf)

st_write(indata_sf, paste0(source1_dir, "Global_2020_HydrothermalVents_InterRidgeVentsDatabasev3.4/Global_2020_HydrothermalVents_InterRidgeVentsDatabasev3.4_crop.shp"), delete_layer = TRUE)
...
path_work <- work_path
source1_path <- path <- paste(source_dir, "Global_2020_HydrothermalVents_InterRidgeVentsDatabasev3.4/Global_2020_HydrothermalVents_InterRidgeVentsDatabasev3.4_crop.shp", sep="")



# write empty grid for gdalutil work
writeRaster(grid_250m_na, paste(path_work, "grid_250m_hydro_vents.tif", sep= ""),  overwrite=T, COMPRESS=LZW)
writeRaster(grid_1km_na, paste(path_work, "grid_1km_hydro_vents.tif", sep= ""),  overwrite=T, COMPRESS=LZW)
```
...
sf_warp <- gdalUtils::gdal_rasterize(src_datasource = source1_path,
                                     dst_filename = paste(path_work, "grid_250m_hydro_vents.tif", sep= ""),
                                     b = 1,
                                     at = F,
                                     a = "Area",
                                     output_Raster = TRUE,
...
```{r}
hydro_vents_250m <- sf_warp
crs(hydro_vents_250m) <- crs(grid_1km)
(hydro_vents_250m)

plot(hydro_vents_250m) # ready to be exported if OK
```


```{r}
# shapefile to 1km grid
sf_warp <- gdalUtils::gdal_rasterize(src_datasource = source1_path,
                                     dst_filename = paste(path_work, "grid_1km_hydro_vents.tif", sep= ""),
                                     b = 1,
                                     at = F,
                                     a = "Area",
                                     output_Raster = TRUE,
...
```{r}
hydro_vents_1km <- sf_warp
crs(hydro_vents_1km) <- crs(grid_1km)
(hydro_vents_1km)


plot(hydro_vents_1km) # ready to be exported if OK
```
...
## read objects
```{r}
product_orig <- hydro_vents_250m # input and check your object to be written to file
...
```{r}
scale <- "250" # "1km" / "250m"
unit <- "m"  # add unit for product with original values (e.g. m, km, perc, presence, proportion
...
```{r}
descriptive_name <- "hydrothermal_vents"  # name of product, for pressure/ecosystem components select from name list above
```
## paths
```{r}
...
# read objects
```{r}
product_orig <- hydro_vents_1km # input and check your object to be written to file
#product_norm01 <- REPLACE # input and check your object to be written to file
#product_uncertainty <- REPLACE # input and check your object to be written to file
...
```{r}
scale <- "1km" 
unit <- "km"  # add unit for product with original values (e.g. m, km, perc, presence, proportion
