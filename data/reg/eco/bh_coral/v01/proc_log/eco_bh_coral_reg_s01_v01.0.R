...
#### ABOUT ####
# ========================================================================================================= #
# Purpose: Script to make map of photic corals
#
# Brief description: 
...
#allen_habitat_250m_r <- raster(paste(path_work, "grid_250m_na_all_habitat_allen_full.tif", sep= ""))
#summary(allen_habitat_250m_r)
...
# add 0 to marine cells in raster
grid_250m_0 <- grid_250m
grid_250m_0[grid_250m_0==1] <- 0
coral_250m_combined_nona <- merge(grid_250m_0, coral_250m_combined)



# write 250m raster - coral presence - to workfile
writeRaster(coral_250m_combined_nona, paste(path_work, "grid_250m_na_coral_combined_nona_lzw.tif", sep= ""), overwrite=T, COMPRESS=LZW, datatype = 'INT2S')
coral_250m_combined_nona <- raster(paste(path_work, "grid_250m_na_coral_combined_nona_lzw.tif", sep= ""))

# aggregate and write 1km raster to workfile

combined_coral_250m_agg1km <- aggregate(coral_250m_combined_nona, fact=4, fun=sum) / 16 #4*4=16, i.e. divide by the maximum cell number to get proportions

#combined_coral_250m_agg1km_norm01 <- combined_coral_250m_agg1km/maxValue(combined_coral_250m_agg1km)

writeRaster(combined_coral_250m_agg1km, paste(path_work, "grid_1km_na_coral_combined_nona_proportion250m.tif", sep= ""),  overwrite=T, COMPRESS=LZW)


combined_coral_250m_agg1km_binary <- aggregate(coral_250m_combined_nona, fact=4, fun=max) 
writeRaster(combined_coral_250m_agg1km_binary, paste(path_work, "grid_1km_na_coral_combined_nona.tif", sep= ""),  overwrite=T, COMPRESS=LZW)
...
r_mosaic <- mask(r_mosaic, grid_1km)
...
# optional.. add nodata areas to coral raster---------------------------------------------------------------------------------------
# create values for noadata areas...
nodata_area <- raster(paste(path_work, "grid_1km_coral_combined2_uncertainty_", version, ".tif", sep= ""))
nodata_area[nodata_area!=5]<- 0 
nodata_area[nodata_area==5]<- 1 

nodata_area_depth <- nodata_area * depth_1km

nodata_area_depth_norm <- nodata_area_depth
nodata_area_depth_norm[nodata_area_depth_norm >-40] <- -40 
nodata_norm <- nodata_area_depth_norm/maxValue(nodata_area_depth_norm)
nodata_norm <- nodata_norm*-1
nodata_norm <- nodata_norm - minValue(nodata_norm)
nodata_norm <- nodata_norm/maxValue(nodata_norm)
nodata_norm <- nodata_norm * nodata_area
plot(nodata_norm)


combined_coral_250m_agg1km <- raster(paste(path_work, "grid_1km_na_coral_combined_nona_proportion250m.tif", sep= ""))

combined_coral_250m_agg1km_unknown <- combined_coral_250m_agg1km + nodata_norm*0.11
combined_coral_250m_agg1km_unknown <- combined_coral_250m_agg1km_unknown/maxValue(combined_coral_250m_agg1km_unknown)
...
writeRaster(product, product_path, COMPRESS=LZW, datatype = 'FLT4S', overwrite=TRUE)


combined_coral_250m_agg1km_unknown

product2_path <- product_path <- paste(dest1_path, theme1, "_", subtheme1, "_reef_proportion_incl_unknown_norm01", "_1km_", version, d_version, ".tif", sep="" )
product_path

# write to file 
write_tsv(file_sourcesym, paste(product_path, "_sourcesym.txt", sep=""))  # write unique sourcesym files for all products
product <- combined_coral_250m_agg1km_unknown # input and check your object to be written to file
writeRaster(product, product_path, COMPRESS=LZW, datatype = 'FLT4S', overwrite=TRUE)
...
product3_path <- product_path <- paste(dest1_path, theme1, "_", subtheme1, "_reef_presence_nona", "_250m_", version, d_version, ".tif", sep="" )
product_path

# write to file 
write_tsv(file_sourcesym, paste(product_path, "_sourcesym.txt", sep=""))  # write unique sourcesym files for all products
product <- coral_250m_combined_nona # input and check your object to be written to file
writeRaster(product, product_path, COMPRESS=LZW, datatype = 'INT4S', overwrite=TRUE)
