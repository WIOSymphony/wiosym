...
# ========================================================================================================= #
## Brief description:
This script combines substrate model from dbSeabed, gebco 2022 bathymetry and SST and SBT data from copernicus to develop benthic habitats based on substrate, depth zone and temperate/tropical oceanography. 
### Scripts/processes that needs to be updated prior?: 
env_topo_reg_s01_v01.2.Rmd (bathymetry model)
...
## DIR1: 
```{r}
source1_dir <- source_dir <-  "./data_raw/reg/env/geo_sub/dbseabed/gk2303301623/"  # REPLACE - internal path
source1 <- source_id <- "gk2303301623" # REPLACE
(read_tsv(paste(source_dir, source_id, "_metasym.txt", sep=""))) # check the metasym file
``` 

```{r}
dir(source_dir, recursive=F)
dir(paste(source_dir, "/Mar2023_Update", sep=""))
```

```{r}
# Files
source1_path <- path <- paste(source_dir, "Mar2023_Update/wio_HardBotm_CODA_IDW3D.asc", sep="")
hard_r <- raster(path) # REPLACE object_name
plot(hard_r)

source2_path <- path <- paste(source_dir, "Mar2023_Update/wio_SoftBotm_CODA_IDW3D.asc", sep="")
...
```{r}
data3_dir <- data_dir <-  "./data/reg/env/ocean/v02/"  # input your data directory
dir(data_dir) # check what data is in this directory
...
data_file <- "trop_limit01_sbt_1km.tif" # your data file
data3_sourcesym2 <- read_tsv(paste(data_dir, data_file, "_sourcesym.txt", sep=""))
data3_path2 <- data_path <- paste(data_dir, data_file, sep="")
trop_sbt_1km <- raster(data_path) # read in your data object if appropriate and perhaps change name to more informative...
plot(trop_sbt_1km)

data_file <- "temp_limit01_sst_1km.tif" # your data file
data3_sourcesym <- read_tsv(paste(data_dir, data_file, "_sourcesym.txt", sep=""))
data3_path <- data_path <- paste(data_dir, data_file, sep="")
temp_1km <- raster(data_path) # read in your data object if appropriate and perhaps change name to more informative...
plot(temp_1km)

data_file <- "temp_limit01_sbt_1km.tif" # your data file
data3_sourcesym2 <- read_tsv(paste(data_dir, data_file, "_sourcesym.txt", sep=""))
data3_path2 <- data_path <- paste(data_dir, data_file, sep="")
temp_sbt_1km <- raster(data_path) # read in your data object if appropriate and perhaps change name to more informative...
plot(temp_sbt_1km)
...
trop_sbt <- trop_sbt_1km
temp_sbt <- temp_sbt_1km
...
r <- hard_shelf_temp <- hard_1km * stack_zone_1km[[2]] * temp_sbt * 100
plot(r)
r <- hard_shelf_trop <- hard_1km * stack_zone_1km[[2]] * trop_sbt * 100
plot(r)
r <- soft_shelf_temp <- soft_1km * stack_zone_1km[[2]] * temp_sbt * 100
plot(r)
r <- soft_shelf_trop <- soft_1km * stack_zone_1km[[2]] * trop_sbt * 100
plot(r)
...
all_sources <- data_raw_sources %>% 
  ...
  filter(id!="gk2104301347") %>% 
  unique() %>% 
  print()

# sources for sst/batht/sub combo (shallow habitats)
sel_sources <- all_sources %>% 
  filter(id!="byq2303302022") %>% 
  print()
# sources for sbt/batht/sub combo (shallow habitats)
sel0_sources <- all_sources %>% 
  filter(id!="byq2303302024") %>% 
  print()
  
# sources for bathy/substrate combo
sel1_sources <- all_sources %>% 
  filter(id!="byq2303302024") %>% 
  filter(id!="byq2303302022") %>% 
  print()
  
#sources for substrate only
sel2_sources <- sel1_sources %>% 
  filter(id=="gk2303301623") %>% 
  print()
...
writeRaster(product_orig13, product_orig_path13, COMPRESS=LZW, datatype = 'FLT4S', overwrite=TRUE)
...
write_tsv(sel_sources, paste(product_orig_path, "_sourcesym.txt", sep=""))  
write_tsv(sel_sources, paste(product_orig_path2, "_sourcesym.txt", sep=""))  
write_tsv(sel_sources, paste(product_orig_path3, "_sourcesym.txt", sep=""))  
write_tsv(sel_sources, paste(product_orig_path4, "_sourcesym.txt", sep=""))  
write_tsv(sel0_sources, paste(product_orig_path5, "_sourcesym.txt", sep=""))  
write_tsv(sel0_sources, paste(product_orig_path6, "_sourcesym.txt", sep=""))  
write_tsv(sel0_sources, paste(product_orig_path7, "_sourcesym.txt", sep=""))  
write_tsv(sel0_sources, paste(product_orig_path8, "_sourcesym.txt", sep=""))  
write_tsv(sel1_sources, paste(product_orig_path9, "_sourcesym.txt", sep=""))  
write_tsv(sel1_sources, paste(product_orig_path10, "_sourcesym.txt", sep=""))  

write_tsv(sel1_sources, paste(product_orig_path11, "_sourcesym.txt", sep=""))  
write_tsv(sel1_sources, paste(product_orig_path12, "_sourcesym.txt", sep=""))  

write_tsv(sel2_sources, paste(product_orig_path13, "_sourcesym.txt", sep=""))  
write_tsv(sel2_sources, paste(product_orig_path14, "_sourcesym.txt", sep=""))  
