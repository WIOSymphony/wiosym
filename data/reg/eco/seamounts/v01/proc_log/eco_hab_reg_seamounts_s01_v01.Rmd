...
plot(indata_sf)
st_write(indata_sf, paste0(source1_dir, "YessonEtAl2019-Seamounts-V2/YessonEtAl2019-Seamounts-V2_crop.shp"), delete_layer = TRUE)
...
path_work <- work_path
source1_path <- path <- paste(source_dir, "YessonEtAl2019-Seamounts-V2/YessonEtAl2019-SeamountBases-V2_crop.shp", sep="")



# write empty grid for gdalutil work
writeRaster(grid_250m_na, paste(path_work, "grid_250m_seamounts.tif", sep= ""),  overwrite=T, COMPRESS=LZW)
writeRaster(grid_1km_na, paste(path_work, "grid_1km_seamounts.tif", sep= ""),  overwrite=T, COMPRESS=LZW)
```
...
sf_warp <- gdalUtils::gdal_rasterize(src_datasource = source1_path,
                                     dst_filename = paste(path_work, "grid_250m_seamounts.tif", sep= ""),
                                     b = 1,
                                     at = F,
                                     a = "Type",
                                     output_Raster = TRUE,
...
plot(seamounts_250m) # ready to be exported if OK
```
...
sf_warp <- gdalUtils::gdal_rasterize(src_datasource = source1_path,
                                     dst_filename = paste(path_work, "grid_1km_seamounts.tif", sep= ""),
                                     b = 1,
                                     at = F,
                                     a = "Type",
                                     output_Raster = TRUE,
...
```{r}
seamounts_1km <- sf_warp
crs(seamounts_1km) <- crs(grid_1km)
(seamounts_1km)


plot(seamounts_1km) # ready to be exported if OK
```
...
## read objects
```{r}
product_orig <- seamounts_250m # input and check your object to be written to file
...
# read objects
```{r}
product_orig <- seamounts_1km # input and check your object to be written to file
#product_norm01 <- REPLACE # input and check your object to be written to file
#product_uncertainty <- REPLACE # input and check your object to be written to file
