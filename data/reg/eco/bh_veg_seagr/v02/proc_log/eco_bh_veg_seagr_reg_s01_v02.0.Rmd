...
### Scripts/processes that needs to be updated prior?: depth, corals, (substrate?)
###
...
source2_id <- source_id <- "gk2304132229" # cut and past your source id here
...
```{r}
data_raw_metasym_num <- 5 # REPLACE - set total number of data_raw dir used (e.g. <- 1), to help in export section
```
...
#(indata_paths <- c(wcmc_sg_pt_250m_path, wcmc_sg_250m_masked_path, kslof_habitat_250m_path, #kslof_seagrass_250m_path, lof_habitat_250m_path, lof_seagrass_250m_path, #allen_habitat_250m_path, allen_seagrass_250m_path))

#(indata_paths <- c(wcmc_sg_pt_250m_path, wcmc_sg_250m_masked_path, allen_habitat_250m_path, #allen_seagrass_250m_path))




# write empty grid for gdalutil output
sg_250m_path <- output_path <- paste(work_path, "sg_250m_na4.tif", sep= "")
writeRaster(grid_250m_na, output_path,  overwrite=T, COMPRESS=LZW)

mosaic_r <- gdalUtils::mosaic_rasters(
  gdalfile = c(wcmc_sg_pt_250m_path, wcmc_sg_250m_masked_path, kslof_habitat_250m_path, allen_habitat_250m_path,
  kslof_seagrass_250m_path, lof_habitat_250m_path, lof_seagrass_250m_path, allen_seagrass_250m_path),
...
r <- raster(sg_250m_path)

sg_250m_lzw_path <- output_path <- paste(work_path, "grid_250m_seagrass_combined3_lzw.tif", sep= "")

writeRaster(r, output_path,  overwrite=T, datatype = 'INT4S', COMPRESS=LZW)
sg_250m_r <- raster(output_path)


sg_250m_r_zero <- merge(sg_250m_r, grid_250m_0)

sg_250m_zero_path <- output_path <- paste(work_path, "grid_250m_seagrass_combined3_zero.tif", sep= "")
writeRaster(sg_250m_r_zero, output_path,  overwrite=T, datatype = 'INT4S', COMPRESS=LZW)

sg_250m_r_zero <-raster(sg_250m_zero_path)
plot(sg_250m_r_zero)
```

alternative way to mosaic
```{r}
(indata_r <- c(wcmc_sg_pt_250m_r, wcmc_sg_250m_r, kslof_habitat_250m_r, kslof_seagrass_250m_r, lof_habitat_250m_r, lof_seagrass_250m_r, allen_habitat_250m_r, allen_seagrass_250m_r))

comb_mosaicT <- mosaic(wcmc_sg_pt_250m_r, wcmc_sg_250m_r, kslof_habitat_250m_r, kslof_seagrass_250m_r, lof_habitat_250m_r, lof_seagrass_250m_r, allen_habitat_250m_r, allen_seagrass_250m_r, fun=last)
comb_mosaicF <- mosaic(wcmc_sg_pt_250m_r, wcmc_sg_250m_r, kslof_habitat_250m_r, kslof_seagrass_250m_r, lof_habitat_250m_r, lof_seagrass_250m_r, allen_habitat_250m_r, allen_seagrass_250m_r, fun=last, na.rm = F)
...
sg_250m_r_zero <- r <- raster(sg_250m_zero_path)
sg_1km_proportion <- aggregate(r, fact=4, fun=sum) / 16 #4*4=16, i.e. divide by the maximum cell number to get proportions
#crs(sg_1km_proportion) <- crs(grid_1km)
(sg_1km_proportion)
plot(sg_1km_proportion) # ready to be exported if OK

r <- sg_1km_proportion
sg_1km_proportion_path <- output_path <- paste(work_path, "grid_1km_seagrass_proportion.tif", sep= "")
writeRaster(r, output_path,  overwrite=T, datatype = 'FLT4S', COMPRESS=LZW)



# adding Saya del Mahla bank data

saya_1km <- raster(saya_1km_path)

sg_1km_proportion_saya <- r <- sg_1km_proportion +  saya_1km/100
sg_1km_proportion_saya_path <- output_path <- paste(work_path, "grid_1km_seagrass_proportion_saya.tif", sep= "")
writeRaster(r, output_path,  overwrite=T, datatype = 'FLT4S', COMPRESS=LZW)


(sg_1km_proportion_saya)
(plot(sg_1km_proportion_saya))  # "final" layer if looks good
...
sg_1km_proportion_zero <- merge(sg_1km_proportion_saya, grid_1km_0)
...
# adjust double counting of points (saya) and polygons (wcmc) >1

r <- sg_1km_proportion_zero
r[r>=1] <- 1

sg_1km_proportion_zero <- r
...
sg_250m_zero <- raster(sg_250m_zero_path)
...
writeRaster(seagrass_1km , paste("./data/reg/eco/bh/v02/proc/", "seagrass_1km_sayamodel_percent_addedpoints.tif", sep=""), overwrite=F, COMPRESS=LZW)

seagrass_1km <- raster(paste("./data/reg/eco/bh/v02/proc/", "seagrass_1km_sayamodel_percent_addedpoints.tif", sep=""))
...
(data_raw_sources <- tibble(id = c(source2, source1, source4, source5, source6))) # REPLACE / add all data_raw sources
```
### data sources (files)
```{r}
