...
# combine wcmc with saya del Mahla bank model... take averge of both maps, hard to tell which is worse...
...
# adding Saya del Mahla bank point data  - not used, instead model above
...
writeRaster(seagrass_1km , paste("./data/reg/eco/bh/v02/proc/", "seagrass_1km.tif", sep=""), overwrite=F, COMPRESS=LZW)
```
...
read objects
```{r}
product_orig <- seagrass_1km # input and check your object to be written to file
plot(product_orig)
```
set names
```{r}
...
# PRODUCT 2: SG 250m - not year done for v2... check
```{r}
