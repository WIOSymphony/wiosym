...
# Use this chunk if creating a SDM for a given taxa that is not the final product,
# but is used to create the final product, e.g. a SDM for a single species
# to be used for a species richness map
```{r}
tax_name <- 'Etmopterus'
```
...
# Selachii = 368408
# Lamniformes = 10209
# Carcharhiniformes = 10210
# Squaliformes = 10211
# Squatiniformes = 10213
```{r}
# IF ONLY ONE TAXON GROUP IS DESIRED
spec <- occurrence(taxonid = 10211,
...
```{r}
temp <- as.data.frame(species)
bgn <- nrow(temp[!duplicated(temp),]) * 5
if(bgn > 1000){bgn <- nrow(temp[!duplicated(temp),])}
```
...
d
m1 <- sdm(Occurrence~.,data=d,methods=c('brt','svm','rf'))
m1
...
prod_cut <- product1
(dest_temp <- paste0(dest1_path, "/", subtheme1, "_", name1, "_", tax_name, "_cutoff_", version, d_version, ".tif"))
...
product1[is.na(grid_1km)==TRUE] <- NA
prod_nocut <- product1
(dest_temp <- paste0(dest1_path, "/", subtheme1, "_", name1, "_", tax_name, "_nocutoff_", version, d_version, ".tif"))
...
```{r}
#base <- grid_1km - 1
eco1 <- raster("./data/reg/eco/cfish_shark/shark_sdm/v02/cfish_shark_shark_sdm_Alopias_cutoff_v02.0.tif")
eco2 <- raster("./data/reg/eco/cfish_shark/shark_sdm/v02/cfish_shark_shark_sdm_Carcharhinus_cutoff_v02.0.tif")
eco3 <- raster("./data/reg/eco/cfish_shark/shark_sdm/v02/cfish_shark_shark_sdm_Carcharodon_cutoff_v02.0.tif")
eco4 <- raster("./data/reg/eco/cfish_shark/shark_sdm/v02/cfish_shark_shark_sdm_Etmopterus_cutoff_v02.0.tif")
eco5 <- raster("./data/reg/eco/cfish_shark/shark_sdm/v02/cfish_shark_shark_sdm_Galeocerdo_cutoff_v02.0.tif")
eco6 <- raster("./data/reg/eco/cfish_shark/shark_sdm/v02/cfish_shark_shark_sdm_Isurus_cutoff_v02.0.tif")
eco7 <- raster("./data/reg/eco/cfish_shark/shark_sdm/v02/cfish_shark_shark_sdm_Sphyrna_cutoff_v02.0.tif")
eco8 <- raster("./data/reg/eco/cfish_shark/shark_sdm/v02/cfish_shark_shark_sdm_Squalus_cutoff_v02.0.tif")
eco9 <- raster("./data/reg/eco/cfish_shark/shark_sdm/v02/cfish_shark_shark_sdm_Squatina_cutoff_v02.0.tif")

eco <- eco1+eco2+eco3+eco4+eco5+eco6+eco7+eco8+eco9
product1 <- eco

#qs <- quantile(eco[eco>0], probs=seq(0,0.95,0.05))# add quantile cutoff?
#qs_cut <- qs["0%"]
#eco[eco < qs_cut] <- 0
#product1 <- eco
...
# Add kenya specific models to map

```{r}
eco[] <- scales::rescale(eco[], to = c(0, 100))

ken <- raster("./data/nat/kenya/eco/cfish_shark/shark_sdm/v01/cfish_shark_nat_kenya_v01.0.tif")
ken_temp <- resample(ken, grid_1km)
ken <- ken_temp
ken_grid <- (ken*0)+1
eco_grid <- eco
eco_grid[is.na(ken)] <- NA
m <- round(maxValue(eco_grid))

ken[] <- scales::rescale(ken[], to = c(0, m))
eco[is.na(ken)== FALSE] <- ken[is.na(ken)== FALSE]
product1 <- eco
...
prod1_scale[] <- scales::rescale(prod1_scale [], to = c(0, 100))
