...
# ========================================================================================================= #
## Brief description: 
Cold water coral model 2023, 
New updated models in press from Tong R et. al. 
There are 10 global models of deep-sea coral species:
D. pertussum, M. oculata, E. rostrata, G. dumosa, O. varicosa, S. variabilis, P. arborea, P. resedaeformis, A. arbuscula, P. placomus
No futher use of data beyond the WIO composite which is the output script without contacting the authors. 

### Scripts/processes that needs to be updated prior?: 

###
### Script by: Initial, org, Rver: 
GK, SGU, R4.1.1, 230416
### Updated: 
GK, SGU, R4.1.1, 230505, added a depth mask which removes unlikely cold coral predictions in shallow tropical areas
...
```{r}
subtheme <- "dbh_coral" # copy from "subtheme_folder"
```
...
## DIR1: 
```{r}
source1_dir <- source_dir <-  "./data_raw/glo/eco/dbh_coral/lit/gk2304110940/"  # REPLACE - internal path
source1 <- source_id <- "gk2304110940" # REPLACE
...
```{r}
# Files
source1_path <- path <- paste(source_dir, "models/", sep="")


#m1 <- raster(paste(path, "AAGBMRF.tif", sep=""))
#m10 <- raster(paste(path, "SVGBMRF.tif", sep=""))

m_stack <- stack(paste(path, list.files(path, pattern = ".tif$", recursive = T), sep=''))
...
data3_file2<- "wiosym_data_bounding_box_v01.shp"
...
grid_poly <- st_read(data_path) # read in your data object if appropriate and perhaps change 

data3_path2 <- data_path <- paste(data_dir, data3_file2, sep="")
grid_data <- st_read(data_path) # read in your data object if appropriate and perhaps change 
...
data_file <- "bathymetry_zone_0-40m_prop_1km_v01.2.tif" # your data file
data4_sourcesym <- read_tsv(paste(data_dir, "bathymetry_mean_m_1km_v01.2.tif", "_sourcesym.txt", sep=""))
data4_path <- data_path <- paste(data_dir, data_file, sep="")

depth_shallow_1km <- raster(data_path) # read in your data object if appropriate and perhaps change name to more informative...
plot(depth_shallow_1km)


```


## Data 5: Temp/tropical layer
```{r}
data5_dir <- data_dir <-  "./data/reg/env/ocean/v02/"  # input your data directory
dir(data_dir) # check what data is in this directory
data_file <- "trop_limit01_sst_1km.tif" # your data file
data_file2 <- "trop_limit01_sbt_1km.tif"
data5_sourcesym <- read_tsv(paste(data_dir, data_file, "_sourcesym.txt", sep=""))
data5_path <- data_path <- paste(data_dir, data_file2, sep="")
...
# PROCESSING
#==========================================================================================================================#
#COLD CORAL resample raster
```{r}

r <- m_stack

grid_data_reproj1 <- st_transform(grid_data, crs(m_stack))
grid_data_reproj1


m_stack_crop <- r <- crop(r, grid_data_reproj1)
...
# check so focal stat have worked (should not modify model data, only fill gaps)
r_diff <- r3 - r
(r_diff)
...
writeRaster(r, paste(work_path, "10model_sum.tif", sep=""), overwrite=T, COMPRESS=LZW)

writeRaster(r3, paste(work_path, "10model_sum_focalfill.tif", sep=""), overwrite=T, COMPRESS=LZW)
...
```{r}
inraster_path <- paste(work_path, "10model_sum_focalfill.tif", sep="")
inraster_path
...
LP_1km <- gdalUtils::gdalwarp(srcfile = inraster_path,
                                dstfile = dstfile,
                                s_srs = sr,
                                t_srs = tr,
                                tr = c(1000, 1000),
...
m_sum_1km <- r
path <- paste(work_path, "m_sum_focalfill_1km_lzw.tif", sep="")
writeRaster(m_sum_1km , path, overwrite=T, COMPRESS=LZW)

```

# normalise data to remove background noice in model
```{r}
r <- m_sum_1km <- raster(path)

q50 <- quantile(r, probs = c(0.55), type=7,names = FALSE)
q99 <- quantile(r, probs = c(0.995), type=7,names = FALSE)

r_norm <- r
  
r_norm[r_norm <= q50] <- q50  

r_norm[r_norm >= q99] <- q99  

r_norm_0100 <- (r_norm-minValue(r_norm))/(maxValue(r_norm)-minValue(r_norm)) * 100


r_norm_0100

path <- paste(work_path, "m_sum_focalfill_1km_lzw_r_norm.tif", sep="")
writeRaster(r_norm_0100 , path, overwrite=T, COMPRESS=LZW)
...
## Adjust Coral with depth - old v1 - not needed with new models
```{r}



m_sum_1km <- raster(path)

depth_mask <- 1-(depth_shallow_1km * trop_1km) # only applying mask in tropical waters since cold corals can occur shallow in temp regions

m_sum_adjusted <- m_sum_1km * depth_mask

plot(m_sum_1km - m_sum_adjusted)

path <- paste(work_path, "m_sum_focalfill_1km_lzw_r_norm_depthadjust.tif", sep="")
writeRaster(m_sum_adjusted , path, overwrite=T, COMPRESS=LZW)
...
## data sources (files  - no data sources should be used for citation in this one (depth/temp not used in v2)
```{r}
...
## read objects
```{r}
product_orig <- raster(paste(work_path, "m_sum_focalfill_1km_lzw.tif", sep="")) # input and check your object to be written to file
product_norm0100 <- m_sum_adjusted # input and check your object to be written to file
#product_uncertainty <- REPLACE # input and check your object to be written to file
plot(product_orig)
plot(product_norm0100)
```
...
```{r}
scale <- "1km" # "1km" / "250m"
unit <- "probability"  # add unit for product with original values (e.g. m, km, perc, presence, proportion
...
```{r}
descriptive_name <- "cold_water_coral"  # name of product, for pressure/ecosystem components select from name list above
```
## paths
```{r}
(product_orig_path <- paste(dest_path, descriptive_name, "_", unit, "_", scale, "_", version, d_version, ".tif", sep="" ))

(product_norm01_path <- paste(paste(dest_path, descriptive_name, "_", unit, "_norm0100_", scale, "_", version, d_version, ".tif", sep="" )))

#(product_uncertainty_path <- paste(product_orig_path, "_uncertainty.tif", sep="" ))
```
...
writeRaster(product_norm0100, product_norm01_path, COMPRESS=LZW, datatype = 'INT4S', overwrite=TRUE)
...
## Export prerry image of product raster(s)
```{r}
(img_path <- paste(dest_path, descriptive_name, "_", unit, "_norm0100_", scale, "_", version, d_version, ".png", sep="" ))
prod1_scale <- product_norm0100
