...
# ========================================================================================================= #
## Brief description: 
Cold water coral model 2022, LOPHELIA model PUBLISHED 2022, more models on the way so later update is likely  
...
## DIR1: 
```{r}
source1_dir <- source_dir <-  "./data_raw/glo/eco/dbh_coral/lit/gk2209011044/"  # REPLACE - internal path
source1 <- source_id <- "gk2209011044" # REPLACE
...
```{r}
# Files
source1_path <- path <- paste(source_dir, "LP_RF.tif", sep="")
LP <- raster(path) # REPLACE object_name
LP
plot(LP)
...
writeRaster(r3, paste(work_path, "LP_rs.tif", sep=""), overwrite=T, COMPRESS=LZW)
...
```{r}
inraster_path <- paste(work_path, "LP_rs.tif", sep="")
inraster_path
...
writeRaster(LP_1km , paste(work_path, "LP_1km.tif", sep=""), overwrite=T, COMPRESS=LZW)

```

## Adjust Coral with depth
```{r}
...
## read objects
```{r}
product_orig <- LP_adjusted # input and check your object to be written to file
