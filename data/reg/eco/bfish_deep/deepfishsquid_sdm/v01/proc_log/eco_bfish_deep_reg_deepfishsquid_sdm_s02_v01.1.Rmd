...
spec <- occurrence(taxonid = "325342", 
                   #scientificname = "Ommastrephes bartramii",
...
writeRaster(product1, paste0(work1_path,"/decapodiformes.tif"), overwrite = T)
...
```{r}
#base <- grid_1km - 1
eco <- raster("./data/reg/eco/bfish_deep/deepfishsquid_sdm/v01/proc/decapodiformes.tif")
eco[] <- scales::rescale(eco[], to =c(0,1))
...
```{r}
depth[depth < 0] <- 0
depth[depth > 0 & depth <= 40] <- scales::rescale(depth[depth > 0 & depth <= 40], to = c(0,1))
depth[depth > 40] <- 1
plot(depth)
eco2 <- eco * depth
