...
inraster_path <- source1_path


# write empty grid for gdalutil work
writeRaster(grid_250m_na, paste(work_path, "grid_250m_gdalutil.tif", sep= ""),  overwrite=T, COMPRESS=LZW)

# write to file
dstfile <- paste(work_path, "grid_250m_gdalutil.tif", sep= "")
...
writeRaster(tidal_250m , paste(work_path, "tidal_250m.tif", sep=""), overwrite=T, COMPRESS=LZW)
...
writeRaster(tidal_agg_1km, paste(work_path, "tidal_1km.tif", sep=""), overwrite=T, COMPRESS=LZW)
...
r.max
tidal <- raster(paste(work_path, "tidal_1km.tif", sep="")) / r.max

tidal_temp <- tidal * temp_1km * 100
plot(tidal_temp)
tidal_trop <- tidal * trop_1km * 100
plot(tidal_trop)
...
write_tsv(all_sources, paste(product_temp_path, "_sourcesym.txt", sep=""))  
write_tsv(all_sources, paste(product_trop_path, "_sourcesym.txt", sep=""))  
