...
# ========================================================================================================= #
## Brief description: 
tidal flats (proxy for mudflats) from 30m resolution global dataset of tidal flats, divided into temperate and tropical zone, using gebco 2022 bathymetry to mask additional areas >20m deep on west african coast (bathymetry for wio region had to much uncertainty along coast to improve the tidal model). 
...
mudflat_250m_v01 <- raster(paste(data6_dir, "proc/tidal_250m.tif" , sep=""))
...
```{r}
# mask areas deeper then 20m according to gebco 2022 bathymetry (due to some suspect artifact noticed when visually assessing 30m tidal mosaic in Murray on the west coast of Africa)
# gebco bathymetry not good enough to use for masking in WIO region
...
# create depth mask for the west coast where bathymetry is "good enough"
r0[r0>-20]<- 1
r0[r0<=-20]<- 0 
plot(r0) 

extent(r0) <- extent(grid_250m_0) 


grid_250m_1 <- grid_250m_0
...
tidal_250m_masked <- tidal_250m*depth_mask
...
writeRaster(r_diff_agg_1km, paste(work_path, "r_diff_tidal_1km_v02_mask20m_wc.tif", sep=""), overwrite=T, COMPRESS=LZW)




# aggregate v02 result to 1km grid
r <- tidal_250m_masked
...
plot(tidal_agg_1km)
writeRaster(tidal_agg_1km, paste(work_path, "tidal_1km_0-100_2002-2016_v02_mask20m_wc.tif", sep=""), overwrite=T, COMPRESS=LZW)

```


## mask v01 tidal based on bathymetry
```{r}
# mask areas deeper then 20m on the west coast of africa according to gebco 2022 bathymetry (due to some suspect artifact noticed when visually assessing 30m tidal mosaic in Murray)

depth_mask

tidal_250m_masked_v01 <- mudflat_250m_v01*depth_mask

# aggregate to 1km grid
r <- tidal_250m_masked_v01
tidal_agg_1km_v01 <- raster::aggregate(r, fact=4, fun=sum, na.rm=T)
r <- tidal_agg_1km_v01
r <- merge(r, grid_1km_0)
r <- crop(r, grid_1km)
r <- mask(r, grid_1km)
...
tidal_agg_1km_v01 <- tidal_agg_1km_v01/16 * 100

plot(tidal_agg_1km_v01)
tidal_agg_1km_v01

writeRaster(tidal_agg_1km_v01, paste(work_path, "tidal_1km_0-100_2002-2016_v01_mask20m_wc.tif", sep=""), overwrite=T, COMPRESS=LZW)
...
combined_tidal <- mosaic(tidal_agg_1km, tidal_agg_1km_v01, fun=max)

plot(combined_tidal)
writeRaster(combined_tidal, paste(work_path, "tidal_1km_0-100_2002-2016_v01_v02_mask20m_wc.tif", sep=""), overwrite=T, COMPRESS=LZW)
...
tidal <- raster(paste(work_path, "tidal_1km_0-100_2002-2016_v01_v02_mask20m_wc.tif", sep=""))
plot(tidal)
...
## Sources combined
```{r}
(all_sources <- data_raw_sources %>% add_row(data_sources) %>% filter(id!="byq2110261901") %>%  unique() )

sel_sources <- all_sources %>% filter(id!="gk2104301347")  # remove shoreline grid source 

sel2_sources <- sel_sources %>% filter(id!="byq2303302024") # remove temp/trop grid source
