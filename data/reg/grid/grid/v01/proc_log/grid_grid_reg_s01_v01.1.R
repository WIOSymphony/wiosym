...
# write 50m grid to (work) file
outfile_warped <- paste(work1_path, subtheme1, "_50m_warped_", version, d_version, ".tif", sep="")
#outfile1 <- paste(work1_path, subtheme1, "_50m_rev2",".tif", sep="")
outfile_warped
writeRaster(bat_50m, outfile_warped, COMPRESS=LZW, datatype = 'INT4S', overwrite=TRUE) 
bat_50m <- raster(outfile_warped)
...
#outfile1 <- paste(work1_path, subtheme1, "_50m_rev2",".tif", sep="")
outfile0
writeRaster(bat_50m_mask, outfile0, COMPRESS=LZW, datatype = 'INT4S', overwrite=TRUE) 
...
outfile01
writeRaster(grid_50m_na, outfile01, COMPRESS=LZW, datatype = 'INT4S', overwrite=F)


# 50m grid shoreline ----------------------------------------------------------------------------------
# bonus - extract 250 m pixels which include shoreline (potentially useful as ecocomponent)
bat_50m_shoreline <- mask(bat_50m_crop, grid_box_50m_r2)
...
outfile <- paste(work1_path, subtheme1, "_50m_shoreline_", version, ".2", ".tif", sep="")
outfile
writeRaster(bat_50m_shoreline, outfile, overwrite=TRUE, COMPRESS=LZW, datatype = 'INT4S') 
bat_50m_shoreline <- raster(outfile)
...
file_data_raw_sources <- tibble(id = c("gk2104301347"))

file0_sourcesym <- file_data_raw_sources %>% 
...
write_tsv(file0_sourcesym, paste(dest1_path_file0, "_sourcesym.txt", sep=""))

bat_50m
writeRaster(bat_50m, dest1_path_file0, COMPRESS=LZW, datatype = 'INT4S', overwrite=TRUE)

# file 2
bat_50m_shoreline
# naming template: dest1_path_file1 <-paste(dest1_path, theme1, "_", subtheme1, "_additional_description", "_", version, d_version, ".format" )
dest1_path_file <-paste(dest1_path, subtheme1, "_50m_shoreline", "_", version, d_version, ".tif", sep="")


file_sourcesym <- file0_sourcesym # same sources

dest1_path_file

write_tsv(file_sourcesym, paste(dest1_path_file, "_sourcesym.txt", sep=""))

bat_50m_shoreline

writeRaster(bat_50m_shoreline, dest1_path_file, COMPRESS=LZW, datatype = 'INT4S', overwrite=TRUE)
