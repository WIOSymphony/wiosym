...
# ABOUT
# ========================================================================================================= #
## Brief description: converting airports to pollution...

### Sripts/processes that needs to be updated prior?: made buffers of the points in GM v21 (5km radius)
###
...
```{r}
subtheme <- "lb_other" # copy from "subtheme_folder"
```
...
## DIR1: 
```{r}
source1_dir <- source_dir <-  "./data_raw/glo/act/lb_other/wfp/pz2110251520/"  # REPLACE - internal path
source1_id <- source_id <- "pz2110251520" # 
...
```{r}
# Files
source1_path <- path <- paste(source_dir, "wld_trs_airports_wfp", sep="")
airports <- st_read(path) # REPLACE object_name
...
## DATA 4: buffered airports shapefile
```{r}
data4_dir <- data_dir <-  "./data/reg/act/lb_other/v01/proc/"  # input your data directory

dir(data_dir)

data4_file <- "buffer_5km.shp" # your data file

data4_path <- data_path <- paste(data_dir, data4_file, sep="")

buffer5km <- st_read(data_path) # read in your data object if appropriate and perhaps change name to more informative...
#grid_poly
```
...
sf <- st_transform(sf, crs=crs(grid_1km))

r <- rasterize(sf, grid_1km_na, 100, fun="mean")
...
#r <- reclassify(r, c(0, 2, 100))

w <- focalWeight(r, 10000, "Gauss")
r_focal_5km <- focal(r, w, na.rm=T) 

r_focal_5km2 <- focal(r_focal_5km, w, na.rm=T) 



#w <- focalWeight(r, 1000, "Gauss")
#r_focal_1km <- focal(r, w, na.rm=T) 


r <- r_focal_5km + r_focal_5km2 
plot(r)
r

# making sure output is snapped and cut...
r <- merge(r, grid_1km_0)
r <- crop(r, grid_1km)
r <- mask(r, grid_1km)
airport <- r # ready for export
```
...
(data_raw_sources <- tibble(id = c(source1_id))) # / add all data_raw sources
```
...
## read objects
```{r}
product_orig <- airport # input and check your object to be written to file
...
```{r}
descriptive_name <- "air_traffic_airport"  # name of product, for pressure/ecosystem components select from name list above
```
## paths
```{r}
